...
# --- HARDCODED PROJECT ROOT TO FIX "None/assets/..." ERRORS ---
PROJECT_ROOT = "/home/umz/unitree_sim_isaaclab"
...
class RobotBaseCfg:
    ...
    def get_base_config(
        ...
        # --- FIX FOR BROKEN PATHS STARTING WITH 'None/' ---
        # This block manually corrects the path using the hardcoded PROJECT_ROOT
        if base_config is not None and hasattr(base_config, "spawn") and hasattr(base_config.spawn, "usd_path"):
            current_path = str(base_config.spawn.usd_path)
            if current_path.startswith("None"):
                base_config.spawn.usd_path = current_path.replace("None", PROJECT_ROOT)
        # --------------------------------------------------
